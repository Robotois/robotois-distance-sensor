...
MyObject::MyObject(uint8_t header = 0) {
  distance = new DistanceSensor(header);
}
...
void MyObject::getValue(const FunctionCallbackInfo<Value>& args){
  ...
  args.GetReturnValue().Set(Number::New(isolate, obj->distance->getValue()));
}

void MyObject::getBasicValue(const FunctionCallbackInfo<Value>& args){
  Isolate* isolate = args.GetIsolate();

  MyObject* obj = ObjectWrap::Unwrap<MyObject>(args.Holder());

  args.GetReturnValue().Set(Number::New(isolate, obj->distance->getBasicValue()));
}
